...
# Теперь импортируем модули через абсолютные пути
from src.core.config import Config
from src.core.models import ProcessingMetrics, FaceRecord
from src.core.data_parser import parse_batch_records, get_global_parser
from src.core.checkpoint_manager import CheckpointManager
from src.core.statistics import StatisticsAnalyzer
...
class OptimizedProgressTracker:
    """Оптимизированный трекер прогресса с минимальным оверхедом"""
    def __init__(self, total_records: int):
        ...
        # Увеличим интервал обновления для снижения оверхеда
        self.update_interval = 5.0  # Обновляем раз в 5 секунд вместо 0.5

    def update(self, processed: int, batch_size: int = 0, memory_usage_mb: float = 0):
        """Обновить прогресс с минимальным оверхедом"""
        self.processed = processed
        current_time = time.time()
        time_since_last = current_time - self.last_update_time
        records_since_last = processed - self.last_update_count

        # Обновляем статистику скорости только раз в update_interval
        if time_since_last >= self.update_interval and records_since_last > 0:
            speed = records_since_last / time_since_last
            self.speeds.append(speed)
            if len(self.speeds) > 10:  # Уменьшили размер буфера
                self.speeds.pop(0)
        ...
        if batch_size > 0:
            batch_time = time_since_last
            self.batch_times.append((batch_size, batch_time))
            if len(self.batch_times) > 5:  # Уменьшили размер буфера
        ...
        if memory_usage_mb > 0:
            ...
            if len(self.memory_samples) > 20:  # Уменьшили размер буфера
                self.memory_samples.pop(0)

    def get_progress_string(self, metrics: ProcessingMetrics) -> str:
        """Получить строку прогресса с минимальным оверхедом"""
        ...
        if self.records_per_second > 0:
            ...
            # Сохраняем ETA для сглаживания
            eta_total_seconds = eta_hours * 3600 + eta_minutes * 60 + eta_seconds
            self.eta_history.append(eta_total_seconds)
            if len(self.eta_history) > 3:  # Уменьшили размер буфера
                self.eta_history.pop(0)

            # Используем среднее значение ETA
            if self.eta_history:
                avg_eta = sum(self.eta_history) / len(self.eta_history)
                eta_hours = int(avg_eta // 3600)
                eta_minutes = int((avg_eta % 3600) // 60)
                eta_seconds = int(avg_eta % 60)

            eta_str = f"{eta_hours:02d}:{eta_minutes:02d}:{eta_seconds:02d}"
        else:
            eta_str = "??:??:??"

        # Использование памяти - делаем это реже
        if time.time() - self.last_update_time < 10:  # Проверяем память не чаще раза в 10 сек
            memory_usage = psutil.virtual_memory().percent
        else:
            memory_usage = getattr(self, '_cached_memory_usage', 0)
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'cache_hit_rate': None  # Будет заполнено из парсера
        }


class OptimizedBatchProcessor:
    """Оптимизированный обработчик батчей"""
...
class OptimizedMemoryManager:
    ...
    def check_memory_safe(self, additional_mb: float = 0) -> bool:
        ...
        try:
            ...
            if len(self.memory_samples) > 200:  # Уменьшили размер буфера
            ...
            if not safe_percent or not safe_available:
                if time.time() - self.last_cleanup > 60:  # Увеличили интервал очистки
                    self.force_cleanup()
    ...
    def force_cleanup(self):
        ...
        try:
            ...
            for _ in range(2):  # Уменьшили количество проходов
                gc.collect()
...
class OptimizedFaceRecognitionProcessor:
    ...
    async def _load_checkpoint_state(self, input_file: str, total_lines: int) -> Tuple[int, Dict[str, Any]]:
        """Загрузить состояние чекпоинта и вернуть начальную позицию и данные чекпоинта"""
        if not self.resume:
            # Если не в режиме возобновления, начинаем с начала
        ...
        if self.checkpoint_manager:
            ...
            if checkpoint_state and self.checkpoint_manager.validate_checkpoint(input_file):
                ...
                # Восстанавливаем метрики из чекпоинта
                ...
                # Добавляем хэши обработанных записей
                ...
                print("🔄 Режим: НОВАЯ ОБРАБОТКА (чекпоинт не найден или невалиден)")
                return 0, {}
        else:
            print("🔄 Режим: НОВАЯ ОБРАБОТКА (менеджер чекпоинтов не инициализирован)")
            return 0, {}
    ...
    async def process_file(self, input_file: str):
        ...
        try:
            ...
            self.batch_processor = OptimizedBatchProcessor(self.image_processor, self.metrics)
            ...
            # Запуск отображения прогресса (теперь обновляем реже)
            ...
            top_stats = snapshot.statistics('lineno')[:5]  # Уменьшили количество отчетов
    ...
    async def _process_file_optimized(self, input_file: str, total_lines: int, start_position: int):
        ...
        try:
            async with self.image_processor:
                # Используем буферизированное чтение
                buffer_size = 1024 * 1024 * 20  # Увеличили буфер до 20MB
                with open(input_file, 'r', encoding='utf-8', buffering=buffer_size) as f:
                    ...
                    for line in f:
                        ...
                        current_byte_position += len(line.encode('utf-8')) + 1  # +1 для символа новой строки

                        # >>>>> ИСПРАВЛЕНО: УДАЛЕНО дублирующее инкрементирование total_records <<<<<
                        # Теперь total_records увеличивается только внутри parse_record()
                        ...
                        # Обрабатываем батч когда накопится достаточное количество данных
                        if len(batch_data) >= self.batch_size:
                            await self._process_and_update_batch(batch_data, current_byte_position, batch_count)
                            batch_data = []
                            batch_count += 1
    ...
    def _adjust_batch_size_dynamically(self, batch_count: int):
        ...
        try:
            ...
            # Логика регулировки размера батча
            if memory_percent > 85 or available_memory_gb < 0.5:
                # Уменьшаем размер батча при высоком потреблении памяти
                new_batch_size = max(self.min_batch_size, int(self.batch_size * 0.7))
                if new_batch_size != self.batch_size:
                    logger.debug(f"Уменьшен размер батча с {self.batch_size} до {new_batch_size} из-за потребления памяти")
                    self.batch_size = new_batch_size
            elif memory_percent < 60 and available_memory_gb > 1.0:
                # Увеличиваем размер батча при наличии ресурсов
                new_batch_size = min(self.max_batch_size, int(self.batch_size * 1.2))
                if new_batch_size != self.batch_size:
                    logger.debug(f"Увеличен размер батча с {self.batch_size} до {new_batch_size} из-за наличия ресурсов")
                    self.batch_size = new_batch_size

            # Также корректируем размер батча на основе времени обработки
            if self.avg_batch_processing_time > 10:  # Если обработка батча занимает больше 10 секунд
                new_batch_size = max(self.min_batch_size, int(self.batch_size * 0.8))
                if new_batch_size != self.batch_size:
                    logger.debug(f"Уменьшен размер батча с {self.batch_size} до {new_batch_size} из-за медленной обработки")
                    self.batch_size = new_batch_size
            elif self.avg_batch_processing_time < 2:  # Если обработка быстрая
                new_batch_size = min(self.max_batch_size, int(self.batch_size * 1.1))
                if new_batch_size != self.batch_size:
                    logger.debug(f"Увеличен размер батча с {self.batch_size} до {new_batch_size} из-за быстрой обработки")
                    self.batch_size = new_batch_size

        except Exception as e:
            logger.warning(f"Ошибка при динамической настройке размера батча: {e}")
    ...
    async def _process_and_update_batch(self, batch_data: List[Tuple[str, str]], 
        ...
        # Обновление счетчиков: processed_records — обработанные без дубликатов
        processed_in_batch = len(batch_records)  # ← это правильное число: уже без дублей и с обработкой изображений
        self.metrics.processed_records += processed_in_batch
        self.total_batches_processed += 1

        # Обновление прогресса (теперь чаще - каждые 1000 записей вместо 5000)
        if self.metrics.processed_records % 1000 == 0 and self.progress_tracker:
        ...
        if self.processed_since_checkpoint >= Config.CHECKPOINT_INTERVAL:
            self._save_checkpoint(
                os.path.basename(self.checkpoint_manager.state.file_name if self.checkpoint_manager.state else ''),
                self.checkpoint_manager.state.total_lines if self.checkpoint_manager.state else self.metrics.total_records,
                current_position
            )
            self.processed_since_checkpoint = 0
        ...
        # Оптимизация памяти каждые 20 батчей (увеличили интервал)
        if batch_count % 20 == 0:
    ...
    async def _display_optimized_progress(self):
        ...
        while self.is_running:
            try:
                current_time = time.time()
                # Обновляем каждые 5 секунд для уменьшения оверхеда (было 2 сек)
                if current_time - last_update >= 5.0 and self.progress_tracker:
                ...
                await asyncio.sleep(2)  # Увеличили задержку в цикле
            except asyncio.CancelledError:
                break
            except Exception as e:
    ...
    async def _monitor_performance(self):
        ...
        while self.is_running:
            try:
                current_time = time.time()
                # Проверяем каждые 10 секунд (было 5 сек)
                ...
                await asyncio.sleep(3)  # Увеличили задержку в цикле
            except asyncio.CancelledError:
                break
            except Exception as e:
    ...
    def _save_checkpoint(self, input_file: str, total_lines: int, position: int):
        """Сохранить чекпоинт"""
        if self.checkpoint_manager:
            # Добавляем уникальные IP в метрики
            self.metrics.unique_ips = self.unique_ips
            self.checkpoint_manager.save_checkpoint(
                file_name=os.path.basename(input_file),
                total_lines=total_lines,
                processed_lines=self.metrics.total_records,  # ← это число включает дубликаты и ошибки JSON — корректно для чекпоинта
    ...
    async def _generate_reports(self):
        ...
        print(f"🌐 Уникальных IP: {len(self.unique_ips)}")
        ...
        # Генерация отчетов - только HTML для максимальной производительности
        reports_created = []
        ...
        # Убираем генерацию других отчетов для ускорения
        # Это позволяет сосредоточиться на основной задаче - обработке данных
    ...
    async def _load_saved_records(self):
        """Загрузка сохраненных записей из чекпоинта или временных файлов"""
        logger.info("Загрузка сохраненных записей...")
        # Если у нас уже есть записи (в случае новой обработки), просто возвращаем
        if not self.resume and len(self.records) > 0:
            logger.info(f"Используем {len(self.records):,} записей из текущей обработки")
            return

        # Если у нас есть чекпоинт, загружаем из него
        if self.checkpoint_manager and self.checkpoint_manager.state:
            # В режиме возобновления можем загрузить дополнительные данные из чекпоинта
            checkpoint_state = self.checkpoint_manager.state
            logger.info(f"Загружено состояние чекпоинта: {checkpoint_state.processed_lines:,} обработанных записей")
        else:
            logger.info("Чекпоинт не найден или не используется")
...
"""
    ...
    async def _count_lines_optimized(self, file_path: str) -> int:
        ...
        def count_lines_sync():
            count = 0
            buffer_size = 1024 * 1024 * 16  # Увеличили буфер
    ...
    async def _get_file_position_async(self, file_path: str, start_position: int, processed_lines: int) -> int:
        ...
        # Для больших файлов используем приблизительный расчет
        try:
                ...
                if processed_lines > 1000:
                    ...
                    with open(file_path, 'r', encoding='utf-8') as f:
                        ...
                        if lines_read > 0:
                            ...
                            return min(start_position + (processed_lines * avg_line_size), file_size)
...
def get_optimized_processor(formats: List[str], resume: bool = False):
