...
from core.data_parser import DataParser, parse_batch_records, get_global_parser
...
class FaceRecognitionProcessor:
    ...
    async def _load_checkpoint_state(self, input_file: str, total_lines: int):
        """Загрузить состояние чекпоинта"""
        start_position = 0
        
        if self.resume:
            checkpoint = self.checkpoint_manager.load_checkpoint()
            if checkpoint and self.checkpoint_manager.validate_checkpoint(input_file):
                ...
                self.unique_ips = set(checkpoint.unique_ips)
    ...
    async def _process_file_optimized(self, input_file: str, total_lines: int, start_position: int):
        ...
        try:
            async with self.image_processor:
                ...
                buffer_size = 1024 * 1024 * 10  # 10MB буфер
    ...
    async def _generate_reports(self):
        ...
        # Убираем генерацию других отчетов для ускорения обработки
                ...
                if processed_lines > 1000:
                    ...
                    with open(file_path, 'r', encoding='utf-8') as f:
                        ...
                        if lines_read > 0:
                            ...
                            return int(min(start_position + (processed_lines * avg_line_size), file_size))
